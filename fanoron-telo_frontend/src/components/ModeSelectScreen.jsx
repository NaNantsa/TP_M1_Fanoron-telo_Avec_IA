...
export default function ModeSelectScreen({ onStart, onBack }) {
  ...
  // Gestion de la difficulté par IA
  ...
  const updateDifficulty = (player, diff) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6 py-10 fade-in relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-3xl mx-auto flex flex-col items-center">
        ...
        {/* Section Difficultés Dynamique */}
        {selected?.ai && (
          <div className="fade-in w-full space-y-4" style={{
            ...
            {/* CONFIGURATION POUR HUMAIN VS IA (Seulement l'IA O) */}
            {mode === "hvai" && (
              <div>
                <p style={{
                  ...
                  Difficulté de l'IA (Joueur O)
                </p>
                <div style={{ display: "flex", gap: "0.6rem" }}>
                  {DIFFICULTIES.map((d) => {
                    const active = aiDifficulty.O === d.id;
                    return (
                      <button
                        key={d.id}
                        onClick={() => updateDifficulty("O", d.id)}
                        style={{
                          flex: 1,
                      ...
                      >
                        <span style={{ fontSize: "0.85rem", fontWeight: 600, color: active ? "#90c4ea" : "#D6E4F7" }}>
                          {d.label}
                        </span>
            ...
            {/* CONFIGURATION POUR IA VS IA (Les 2 IA séparées) */}
            {mode === "aivai" && (
              <div className="space-y-4">
                ...
                <div>
                  <p style={{
                    ...
                    Difficulté de l'IA X
                  </p>
                  <div style={{ display: "flex", gap: "0.6rem" }}>
                    {DIFFICULTIES.map((d) => {
                      const active = aiDifficulty.X === d.id;
                      return (
                        <button
                          key={d.id}
                          onClick={() => updateDifficulty("X", d.id)}
                          style={{
                            flex: 1,
                            background: active ? "rgba(74,144,217,0.22)" : "rgba(74,144,217,0.05)",
                            border: active ? "1px solid rgba(74,144,217,0.5)" : "1px solid rgba(74,144,217,0.12)",
        ...
        <div style={{ display: "flex", gap: "0.75rem", width: "100%" }}>
          ...
          <button
            onClick={() => onStart(mode, aiDifficulty)}
            style={{
              flex: 1,
              display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
